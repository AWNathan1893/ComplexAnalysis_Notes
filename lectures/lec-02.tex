...
\begin{Def}{Parametrized curve}{param:curve}\index{Parametrized curve}
  \begin{itemize}
    ...
    \item \(\gamma\) is said to be \textbf{simple closed}\index{Simple Closed curve} if \(\gamma\) is closed and one-one. In other words, there is no loop.
  \end{itemize}
\end{Def}
...
The integer \(n\) in the above example is called the \textbf{winding number}\index{Winding number} of the curve \(\gamma\) around \(0\), which is a topological invariant and is studied in detail in index theory.
