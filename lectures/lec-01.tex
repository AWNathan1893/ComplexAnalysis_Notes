...
\begin{enumerate}
  ...
  \item $\{z_n\}_{n \in \N} \subseteq \C$ is a Cauchy sequence if and only if $\{(x_n,y_n)\}_{n \in \N} \subseteq \R^2$ is a Cauchy sequence, which is equivalent to $\{x_n\}_{n\in\N}$ and $\{y_n\}_{n\in\N}$ are Cauchy sequences.
